            ...
            For i As Integer = 0 To lines.Length - 1
                ...
                If Divide(lines(i), 2, "=")(0) = "Hydr Grp ID" Then
                    ...
                    For j As Integer = 0 To NoValve - 1
                        HydraulicGroup(Index).Valve(j).UID = ConvertToInt(Divide(lines(i + 4 * j + 7 + NoPump + NoFlowMeter + NoMoistureProbe), 2, "=")(1))
                        HydraulicGroup(Index).Valve(j).MinThreshold = ConvertToInt(Divide(lines(i + 4 * j + 9 + NoPump + NoFlowMeter + NoMoistureProbe), 2, "=")(1))
                        HydraulicGroup(Index).Valve(j).MaxThreshold = ConvertToInt(Divide(lines(i + 4 * j + 10 + NoPump + NoFlowMeter + NoMoistureProbe), 2, "=")(1))
                    Next
                    Index = Index + 1
                End If
            Next
